...
def init_shapes(num_paths,
    ...
    for i in range(num_paths):
        ...
        if 'gt' in kwargs:
            gt = kwargs['gt'] #?为什么需要这样呢    #bug here
            wref, href = color_ref
    ...
    for group in shape_groups:
        group.fill_color = group.fill_color.cuda()
...
def live(cfg_arg,
         config='code/config/LIVE_base.yaml',
         debug=False,
         initial='circle',
         seginit=None,
         num_segments=4):
    ...
    cfg_default = edict(_cfg['default'])
    _cfg = edict(_cfg[f"experiment_{cfg_arg.optim_path}"])
    _cfg.update(cfg_default)
    cfg = edict()
    cfg.update(cfg_arg)
    cfg.update(_cfg)    #保证自己的设置会覆盖父亲的设置
    
    cfg.debug = debug
    cfg.initial = initial
    if seginit is not None:
        cfg.seginit = edict()
        cfg.seginit.type = seginit[0]
        if cfg.seginit.type == 'circle':
            cfg.seginit.radius = float(seginit[1])
    cfg.num_segments = num_segments

    cfg.exid = get_experiment_id(cfg.debug)

    
    configfile = osp.join(cfg.experiment_dir, 'config/LIVE_config.yaml')
    ...
    png_path = osp.join(cfg.experiment_dir,'init_png',f"{cfg.filename}.png")
    gt = np.array(PIL.Image.open(png_path))
    ...
    for path_idx, pathn in enumerate(path_schedule):
        ...
        print("#{}:".format(path_idx),pathn_record_str)
        ...
        for t in t_range:
            ...
            img = render(w, h, 2, 2, t, None, *scene_args)  #从矢量图渲染图像
        ...
        # shapes_record,shape_groups_record 保存矢量图的数据
    ...
    save_svg_filename = osp.join(cfg.experiment_dir,'init_svg',f"{cfg.filename}.svg")
    check_and_create_dir(save_svg_filename)
    pydiffvg.save_svg(save_svg_filename, w, h, shapes_record, shape_groups_record)
